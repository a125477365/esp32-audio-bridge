...
void AudioSettings::loadFromNVS() {
  ...
  // Load last audio config (for recovery after reboot)
  lastSampleRate = prefs.getULong(NVK_SAMPLE_RATE, DEFAULT_SAMPLE_RATE);
  lastBitsPerSample = prefs.getUChar(NVK_BITS, DEFAULT_BITS_PER_SAMPLE);
  lastChannels = prefs.getUChar(NVK_CHANNELS, DEFAULT_CHANNELS);
...
void AudioSettings::saveToNVS() {
  ...
  // Audio - last used config
  prefs.putULong(NVK_SAMPLE_RATE, lastSampleRate);
  prefs.putUChar(NVK_BITS, lastBitsPerSample);
  prefs.putUChar(NVK_CHANNELS, lastChannels);

  prefs.end();
}

void AudioSettings::saveAudioConfig(uint32_t sampleRate, uint8_t bitsPerSample, uint8_t channels) {
  ...
  Preferences prefs;
  prefs.begin(NVS_NAMESPACE, false);
  prefs.putULong(NVK_SAMPLE_RATE, sampleRate);
  prefs.putUChar(NVK_BITS, bitsPerSample);
  prefs.putUChar(NVK_CHANNELS, channels);
  prefs.end();

  DEBUG_SERIAL.printf("[Settings] Saved audio config: %lu Hz / %d bit / %d ch\n", sampleRate, bitsPerSample, channels);
}
...
void AudioSettings::reset() {
  ...
  lastSampleRate = DEFAULT_SAMPLE_RATE;
  lastBitsPerSample = DEFAULT_BITS_PER_SAMPLE;
  lastChannels = DEFAULT_CHANNELS;
}
...
size_t AudioSettings::calculateBufferSize() {
  // Use last known sample rate if available, otherwise default
  return calculateOptimalBufferSize(lastSampleRate, lastBitsPerSample);
}
